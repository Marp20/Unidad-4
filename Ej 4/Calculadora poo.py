...
class Calculadora(object):
    ...
    def ponerNUMERO(self, numero):
        if self.__operadorAux == None:
            ...
            self.__operadorAux = None
            valor = self.__panel.get()
            self.__primerOperando = Imaginario(float(valor), 0)  # Número imaginario con parte imaginaria cero
    ...
    def resolverOperacion(self, operando1, operacion, operando2):
        ...
        if operacion == '+':
            ...
            resultado = operando1 / operando2
        if self.__flag:
            self.__panel.set(str(resultado.real) + ' + ' + str(resultado.imaginario) + 'i')
        else:
            print(resultado.imaginario)
    def ponerOPERADOR(self, op):
        
        if op == '=':
            operacion = self.__operador.get()
            self.__segundoOperando = Imaginario(float(self.__panel.get()), 0)  # Número imaginario con parte imaginaria cero
